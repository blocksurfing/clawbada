...
1. **`BoostEpochService`** (`apps/engine/src/boost/`) — a 60 s tick calling `runEpochJob`:
   ...
   - For every row whose `ends_at <= now` and is not `activated`, **in window order**,
     ...
     - `computeEpoch` (`announced|active|closing -> computed`): one transaction; played =
       ...
       and Power 3..9 and team not disbanded; every other rated team, disbanded ones included (a disbanded roster's rating is the lineage parent for its successor, so it must keep fading; rows already consumed by a successor are skipped), gets one idle-decay
...
## Verification scripts (run before a deploy that touches this path)

Two scripts exercise the real SQL and the real chain path that the unit tests mock. Both were run on 2026-09-03 against Postgres 16 (migrations 0000-0005) and a local Anvil; the DB run is what caught the disband-freeze loophole (a disbanded team's rating was exempt from idle decay, so disband + recreate with the same three lobsters would have inherited a frozen rating).

**DB-backed** (`packages/db/scripts/verify-boost-db.ts`, WIPES the boost tables — throwaway database only):

```
docker run -d --name pg-verify -p 5433:5432 -e POSTGRES_USER=clawbada -e POSTGRES_PASSWORD=clawbada -e POSTGRES_DB=clawbada postgres:16-alpine
export DATABASE_URL=postgresql://clawbada:clawbada@127.0.0.1:5433/clawbada
(cd packages/db && bun run migrate)
BOOST_EPOCH_ANCHOR_TS=$(( $(date +%s) - 8*24*3600 )) bun run --filter @clawbada/db verify:boost
```

Covers: fresh + repeated `ensureTeamRating`; lineage across disband + recreate (2 shared -> 1/3 regression, parent consumed once, 1 shared with a consumed parent -> fresh); Power change reset; 8 battles of participation + outcome with replay rejection (partial unique indexes, `FOR UPDATE`); `ensureAnnounced` never overwriting a floor; `computeEpoch` (2 qualified, idle decay for live + disbanded-unconsumed teams, consumed parent skipped, idempotent re-run); `stageEpoch` outbox rows keyed `boost:set:<epoch>:<batch>` (re-run reuses them); `activateEpoch` waiting on set jobs, then `boost:activate:<epoch>`, then `activated` with the tx hash; `runEpochJob` and `checkOverdue` afterwards.

**On-chain** (`packages/chain/scripts/verify-boost-onchain.ts`, local Anvil only):

```
anvil --chain-id 84532 --port 8545 --silent &
export DEPLOYER_PRIVATE_KEY=<anvil key 0> DEV_WALLET=<anvil address 1>
forge script contracts/script/Deploy.s.sol    --rpc-url http://127.0.0.1:8545 --broadcast
forge script contracts/script/Configure.s.sol --rpc-url http://127.0.0.1:8545 --broadcast
export CHAIN_ENV=testnet BASE_SEPOLIA_RPC_URL=http://127.0.0.1:8545 OPERATOR_PRIVATE_KEY=<anvil key 0>
export MINING_POOL_ADDRESS=... LOBSTER_NFT_ADDRESS=... TEAM_MANAGER_ADDRESS=... CLAW_TOKEN_ADDRESS=...   # from deployments/base-sepolia.json
bun run --filter @clawbada/chain verify:boost-onchain
rm -rf broadcast deployments/base-sepolia.json   # generated by the local run
```

Covers, through the real `set_team_boosts` / `activate_boost_epoch` handlers: stage for epoch 1 (not live yet), activate (`currentBoostEpoch == 1`, `teamBoostBps(team, power) == 5000`, other power -> 0), re-activation classified as already done, epoch 3 and bps 6000 classified dead, live-epoch amend, and a `startExpedition` whose `ExpeditionStarted` carries `boostBps` with `reward == base x (1 + bps) x tierWeight` and `epochWeightServed` bps-scaled.
