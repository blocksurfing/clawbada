...
# run with -allowPlaceholderAudio (a local test build carrying unlicensed audio), and deletes
...
if [ -f "$MARKER" ]; then
  echo "REFUSING to deploy — the local WebGL build contains unlicensed placeholder audio:" >&2
  sed 's/^/  /' "$MARKER" >&2
